...
namespace MVoxelizer
{
    ...
    public class MeshVoxelizer
    {
        ...
        public bool approximation = false;
        ...
        //separate voxels
        public FillCenterMethod fillCenter = FillCenterMethod.None;
        public Material centerMaterial = null;

        //voxelization data
        [SerializeField] protected MVSource m_source;
        [SerializeField] protected MVGrid m_grid;
        [SerializeField] protected MVResult m_result;
        [SerializeField] protected MVOptimization m_opt;
        [SerializeField] protected Dictionary<MVInt3, MVVoxel> voxelDict = new Dictionary<MVInt3, MVVoxel>();
        ...
        protected virtual bool AnalyzeMesh()
        {
            ...
            switch (precision)
            {
                ...
                case Precision.VeryHigh:
                    triangleStep = m_grid.unitSize * 0.03125f;
                    break;
                default:
            ...
            for (int subMesh = 0; subMesh < m_source.mesh.subMeshCount; ++subMesh)
            {
                ...
                for (int i = start; i < end; i += 3)
                {
                    ...
                    CheckTriangle(i, subMesh, triangleStep);
                }
            }
            return true;
        }
        
        void CheckTriangle(int index, int subMesh, float triangleStep)
        {
            Vector3 p0 = m_source.mesh.vertices[m_source.mesh.triangles[index]];
            Vector3 p1 = m_source.mesh.vertices[m_source.mesh.triangles[index + 1]];
            Vector3 p2 = m_source.mesh.vertices[m_source.mesh.triangles[index + 2]];
            if (ignoreScaling)
            {
                p0.x *= m_source.transform.localScale.x;
                p0.y *= m_source.transform.localScale.y;
                p0.z *= m_source.transform.localScale.z;
                p1.x *= m_source.transform.localScale.x;
                p1.y *= m_source.transform.localScale.y;
                p1.z *= m_source.transform.localScale.z;
                p2.x *= m_source.transform.localScale.x;
                p2.y *= m_source.transform.localScale.y;
                p2.z *= m_source.transform.localScale.z;
            }

            //Vector3 normal = (m_source.mesh.normals[m_source.mesh.triangles[index]]
            //                 + m_source.mesh.normals[m_source.mesh.triangles[index + 1]]
            //                 + m_source.mesh.normals[m_source.mesh.triangles[index + 2]]) / 3.0f;
            ...
            while (pos01 < len01)
            {
                ...
                while (pos02 < len02 * r)
                {
                    ...
                    CheckPoint(p, ratio, index, subMesh, normal);
                    pos02 += triangleStep;
                }
                pos01 += triangleStep;
            }
            ...
            CheckPoint(p2, new Vector3(0.0f, 0.0f, 1.0f), index, subMesh, normal);
        }

        void CheckPoint(Vector3 p, Vector3 ratio, int index, int subMesh, Vector3 normal)
        {
            p -= m_grid.origin;
            MVVoxel voxel;
        ...
        protected virtual bool ProcessVoxelData()
        {
            ...
            float bound = m_grid.unitSize * 0.5f * APPROXIMATION_THRESHOLD;
            foreach (var v in voxelDict)
            {
                        ...
                        for (int z = 0; z < m_grid.unitCount.z; ++z)
                        {
                            ...
                            for (int i = 0; i < indice.Count - 1; ++i)
                            {
                                ...
                                for (int j = indice[i] + 1; j < indice[i + 1]; ++j)
                                {
                                    ...
                                    voxel.right = false;
                                    voxelDict.Add(m_grid.GetGridCoordinate(pos), voxel);
        ...
        protected virtual GameObject GenerateVoxels(List<Vector3> centerVoxels)
        {
            ...
            if (!ignoreScaling) voxelGroup.transform.localScale = sourceGameObject.transform.localScale;
        ...
        protected virtual bool GenerateMeshVertices()
        {
            ...
            {
                int temp = 0;
                GameObject MeshParent = new GameObject("Parent");

                MeshParent meshParent = MeshParent.AddComponent<MeshParent>();
                meshParent.meshVoxelizer = this;

                foreach (MVVoxel voxel in voxelDict.Values)
                {
                    var instantiated = new GameObject($"voxelAto{temp++}");
                    instantiated.transform.SetParent(MeshParent.transform);
                    instantiated.transform.position = voxel.centerPos;
                    instantiated.AddComponent<BoxCollider>();
                    instantiated.AddComponent<MeshData>().voxelData = voxel;
        ...
        public GameObject GenerateResultAgain()
        {
        ...
        public virtual GameObject GenerateResult()
        {
